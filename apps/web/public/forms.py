...
class AttachMentGroupForm(Form):
    ...
    name = StringField("分组名称", validators=[DataRequired(message="请输入分组名称"), Length(min=2,max=10, message="分组名称长度为2-10")])
...
class AttachMentForm(Form):
    ...
    grouid = IntegerField()
    userid = IntegerField("用户代码", validators=[DataRequired(message="请输入用户代码")])
    name = StringField("素材名称", validators=[DataRequired(message="请输入素材名称"), Length(min=2,max=10, message="素材名称长度为2-30")])
    type = StringField("素材类型", validators=[DataRequired(message="请输入素材类型")])
